...
function submitFeedback() {
    ...
    if (starSelected && commentText) {
        ...
        alerta.textContent = `Gracias por tu calificacion de ` + Cantidad + ` estrellas y tu comentario.`;
        ...
        if (starSelected) {
            ...
            alerta.classList.add("alert", "alert-success");
            alerta.textContent = `Gracias por tu calificacion de ` + Cantidad + ` estrellas.`;
            document.body.appendChild(alerta);
            alerta.scrollIntoView();
            // Esperar 2s
            setTimeout(function () {
